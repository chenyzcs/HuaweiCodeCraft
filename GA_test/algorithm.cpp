...
std::vector<std::vector<int>>  Max_Flow_Min_Cost(std::vector<netNode_index> &assign_host_nodes)
{
	...
	while (is_path != INF && req_bandwidth_2_netNode[netNode_nums + 1] > 0)
	{
		int _tmp_cost = 0;
		int _tmp_bw = 0;
		auto _tmp_ans = Dijkstra_max_flow_min_cost(netNode_nums, netNode_nums + 1,
	...
	{
		_total_cost.push_back((total_cost + assign_host_nodes.size()*serverCost)/1);
