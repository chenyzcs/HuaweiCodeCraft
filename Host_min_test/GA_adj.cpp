...
std::vector<netNode_index> _host_map;
std::vector<std::vector<netNode_index>> host_location_map(GA_times);
//generate 20 
int gen(float *min_cost) {
	...
	for (int i = 0; i < netNode_nums / range_const; i++)
	{
		...
		out_degree_per_point_tmp[max_index] = INF;
	...
	{
		bool _same = false;
		for (int j = 0; j < node_sort.size(); j++)
		{
			if (i==node_sort[j])
			{
				_same = true;
	...
	{
		for (int j = 0; j < _host_map.size()/GA_times; j++)
		{
			host_location[i].push_back(_host_map[i + j*GA_times]);// +j*GA_times);
			host_location_map[i].push_back(i + j*GA_times);
		}
		auto x = Max_Flow_Min_Cost(host_location[i]);
		apt[i] = x.back()[0];
	}
	...
	for (int gen_times = 0; gen_times < max_gen_times; gen_times++)
	{
		...
		//}
			...
			for (int i = 0; i < GA_times; i++)
			{
				...
				if (apt[n2]<apt[n1])
				{
					for (int k = 0; k < host_location_map[n2].size(); k++)
					{
						host_location_list[i].push_back(host_location_map[n2][k]);
					}
				}
				else
				{
					for (int k = 0; k < host_location_map[n1].size(); k++)
					{
						host_location_list[i].push_back(host_location_map[n1][k]);
					}
				}
			}
		...
		for (int i = 0; i < GA_times; i++)
		{
			...
			if (cross_rate < cross_p)
			{
				...
				//}
				int n3 = rand_gen_int(0, _host_map.size() - 1);
				for (int j = n3; j <_host_map.size(); j++)
				{
		...
		for (int i = 0; i < GA_times; i++)
		{
			...
			//	}
			//}
			for (int j = 0; j < _host_map.size(); j++)
			{
				float mutation_rate = rand_gen_float();
		...
		for (int i = 0; i < GA_times; i++)
		{
			int tmp_size = host_location[i].size();
			host_location[i].clear();
			for (int j = 0; j < tmp_size; j++)
			{
				if (gen_matrix[i][j] == 1)
				{
					host_location[i].push_back(_host_map[j]);
				}
			}
			...
			if (host_location[i].size()>consumeNode_nums)
			{
				...
				int n = rand_gen_int(1, consumeNode_nums - 1);//random numbers in (0,C-1)
															  //std::cout <<n<<std::endl;
				std::vector<netNode_index> Q(_host_map.size(), 0);

				while (n > 0) {
					int m = rand_gen_int(0, _host_map.size() - 1);
					if (!Q[_host_map[m]]) {
						Q[_host_map[m]] = 1;
						host_location[i].push_back(_host_map[m]);
						n--;
					}
				}
