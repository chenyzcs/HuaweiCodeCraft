...
//F:\Huawei\20170323\case_example\第二批练习用例\0 初级
	const char * filename_c = "F:\\Huawei\\case_example\\2\\case0.txt";
	//const char * filename_c = "F:\\Huawei\\case_example\\case0.txt";
	...
	std::vector<std::vector<netNode_index>> best_connect(netNode_nums, std::vector<int>(1, INF)); 
	cout << "Max_Flow_Min_Cost" << endl;
	start = clock();

	//std::vector<netNode_index> x{ 115,/*122,38,77,*/72,22,158,0,34,150,33,57,24,145,86,5,16,56,23,137,84,91,157,45,81 };
	std::vector<netNode_index> x{ 35 };
	auto dd = Max_Flow_Min_Cost(x, best_connect);
	finish = clock();
	cout << (finish - start) << "ms" << endl;
	cout << dd.back()[0] << endl;
	float a, b;
	...
	start = clock();
	float min_cost;
	gen(&min_cost, best_connect);
	
	auto xx = exchange_host_list(&min_cost, best_connect);
	auto yy = Max_Flow_Min_Cost(xx, best_connect);
