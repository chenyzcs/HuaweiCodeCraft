...
// max flow min cost
std::vector<std::vector<float>>  Max_Flow_Min_Cost(std::vector<netNode_index> &assign_host_nodes, std::vector<std::vector<netNode_index>> &bestcnn)
{
	// 生成动态带宽与租金矩阵
	// N维是服务器源点 N+1维是消费汇点
	...
	int merge_times = 0;
	int total_cost = 0;
	std::vector<std::vector<float>> link_list2;
	std::vector<float> path_pay;
	...
	std::vector<std::vector<netNode_index>> best_connect(netNode_nums, std::vector<int>(1, 0));
	for (int i = 0; i < link_list2.size(); i++)
	{
		int _t_size = link_list2[i].size();
		if (link_list2[i].size()<4 || req_bandwidth_2_netNode[link_list2[i][_t_size - 3]] != 0)	continue;
		else
		{
			best_connect[link_list2[i][_t_size - 3]][0] += (path_pay[i])*(link_list2[i][_t_size - 1]);
			best_connect[link_list2[i][_t_size - 3]].push_back(link_list2[i][0]);
		}
		
	}

	for (int i = 0; i < best_connect.size(); i++)
	{
		if (best_connect[i].size() < 2)	
			continue;

		if (best_connect[i][0] < bestcnn[i][0] && best_connect[i][0] < serverCost)
		{
			bestcnn[i].clear();
			bestcnn[i] = best_connect[i];
		}
	}

	int last_con_nums = 0;
	std::vector<netNode_index> accept_consumeNode;
	std::vector<netNode_index> noaccept_consumeNode;
	int accept_bw_cost = 0;
	int noaccept_bw_cost = 0;
	accept_consumeNode.clear();
	noaccept_consumeNode.clear();
	for (int i = 0; i < consumeNode_nums; i++)
	{
		if (req_bandwidth_2_netNode[consumeNodeList[i]] == 0)
		{
				...
				last_con_nums++;
				accept_consumeNode.push_back(consumeNodeList[i]);
				accept_bw_cost += requireBandWidth[i];
			
			//std::cout << consumeNodeList[i] << " " << requireBandWidth[i] << std::endl;
		}
		else
		{
			noaccept_consumeNode.push_back(consumeNodeList[i]);
			noaccept_bw_cost += requireBandWidth[i];
		}
		//req_bandwidth_2_netNode
	}
	//std::cout << std::endl;
	//std::cout << (consumeNode_nums - last_con_nums)*serverCost - total_cost << std::endl;

	std::vector<netNode_index> accept_netNode;
	accept_netNode.clear();
	int accept_link_cost = 0;
	for (int i = 0; i < accept_consumeNode.size(); i++)
	{
		for (int j = 0; j < link_list2.size(); j++)
		{
			int _t_size = link_list2[j].size();
			if (_t_size > 3 && link_list2[j][_t_size - 3] == accept_consumeNode[i])
			{
				
					accept_netNode.push_back(link_list2[j][0]);
					accept_link_cost += path_pay[j] * link_list2[j].back();
			}
		}
	}
	sort(accept_netNode.begin(), accept_netNode.end());
	accept_netNode.erase(unique(accept_netNode.begin(), accept_netNode.end()), accept_netNode.end());
	auto xxxx = assign_host_nodes;
	assign_host_nodes.clear();
	assign_host_nodes = accept_netNode;
	std::vector<float> _total_cost;
	float ep = 1;
	/*if (((accept_link_cost + accept_netNode.size()*serverCost) - accept_consumeNode.size()*serverCost)<0)
	{*/
		//_total_cost.push_back(((float)(total_cost + assign_host_nodes.size()*serverCost))/(assign_host_nodes.size()*serverCost));
		//_total_cost.push_back(((float)(total_cost + assign_host_nodes.size()*serverCost)));
		//_total_cost.push_back(((float)(assign_host_nodes.size()*serverCost))/(total_cost + assign_host_nodes.size()*serverCost));
		//_total_cost.push_back(((float)(total_cost))/ (consumeNode_nums - assign_host_nodes.size())*(assign_host_nodes.size()*serverCost));
		//_total_cost.push_back(((float)(total_cost + assign_host_nodes.size()*serverCost))/(max_bandwidth_require - req_bandwidth_2_netNode[netNode_nums + 1])+ req_bandwidth_2_netNode[netNode_nums + 1]*1000);
		//if (accept_consumeNode.size()==0)
		//{
		//	_total_cost.push_back(INF);
		//	link_list2.push_back(_total_cost);
		//}
		//else
		//{
	
		float a = (float)(accept_link_cost + accept_netNode.size()*serverCost);
		float b = float(accept_consumeNode.size()*serverCost*accept_netNode.size());

		//float xxx = a / accept_consumeNode.size();
		//float yyy = a + 300*xxx*(consumeNode_nums - accept_consumeNode.size());
		float c = a / b; //+ (accept_bw_cost - noaccept_bw_cost);
		if (b == 0)
		{
			//std::cout << accept_link_cost << " " << accept_netNode.size() << std::endl;
			//std::cout << a << " " << b << " " << c << std::endl;
			_total_cost.push_back(INF-1);
			link_list2.push_back(_total_cost);
		}
		else
		{
			_total_cost.push_back(c);
			link_list2.push_back(_total_cost);
		}
	...
	//			else
	//			{
	//				for (int l = 1; l < bestcnn[k].size(); l++)
	//				{
	//					assign_host_nodes.push_back(bestcnn[k][l]);
	//				}
	//			}
	//			
	//		}
	//			
	//	}
	//	sort(assign_host_nodes.begin(), assign_host_nodes.end());
	//	assign_host_nodes.erase(unique(assign_host_nodes.begin(), assign_host_nodes.end()), assign_host_nodes.end());
	//	return Max_Flow_Min_Cost(assign_host_nodes, bestcnn);
