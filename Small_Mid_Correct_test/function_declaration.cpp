...
std::string output_direct_link()
{
	...
	for (int i = 0; i < consumeNode_nums; i++)
	{
		if (i<consumeNode_nums-1)
		{
			str.push_back(std::to_string(consumeNodeList[i]) + " " + std::to_string(i) + " " + std::to_string(requireBandWidth[i]) + "\n");
		}
		else
		{
			str.push_back(std::to_string(consumeNodeList[i]) + " " + std::to_string(i) + " " + std::to_string(requireBandWidth[i]));
...
unsigned long print_time()
{
	...
	ftime(&rawtime);
	static unsigned long _s = rawtime.time;
	unsigned long out_s = rawtime.time - _s;
	_s = rawtime.time;
	return out_s;
}