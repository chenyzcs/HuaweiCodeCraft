...
//		{
//			int _tmp_ans_len = _tmp_ans[1].size();
//			for (int k = 0; k < _tmp_ans_len; k++)
//			{
//				if (k + 1 < _tmp_ans_len)
//				{
...
//		for (int j = 0; j < assign_length; j++)
//		{
//			if (assign_host_nodes[j] == consumeNodeList[i])
//				dir_host++;
//		}
//	}
...
//	{
//		_total_cost.push_back((total_cost + assign_length*serverCost));
//		//_total_cost.push_back((total_cost + 0.1*host_server_num*serverCost + 100 * (assign_host_nodes.size() - host_server_num)*serverCost));
//		//_total_cost.push_back((total_cost + 0.1*(assign_host_nodes.size()- dir_host)*serverCost + 2.5 * (dir_host)*serverCost));
...
//	else
//	{
//		int host_server_num = assign_length;
//		for (int k = 0; k < netNode_nums; k++)
//		{
//			if (req_bandwidth_2_netNode[k] > 0) {
...
//		int link_list2_len = link_list2.size() - 1;
//		for (int i = 0; i < link_list2_len; i++)
//		{
//			int _s = link_list2[i].size() - 3;
//			for (int j = assign_length - 1; j <host_server_num; j++)
//			{
...
//			}
//		}
//		_total_cost.push_back((total_cost + 0.1*(assign_length - dir_host)*serverCost + 2.5 * (dir_host + host_server_num - assign_length)*serverCost));
...
std::vector<std::vector<int>>  Max_Flow_Min_Cost(std::vector<netNode_index> &assign_host_nodes)
{
	...
	std::vector<std::vector<int>> link_list2;
	std::vector<int> path_pay;
	//auto x = costflow(_flow_G_adj, _flow_cost_rentcost, _flow_cost_bandwidth);
	//std::cout << (x + assign_host_nodes.size()*serverCost) << std::endl;
	while (is_path != INF && req_bandwidth_2_netNode[netNode_nums + 1] > 0)
	{
		int _tmp_cost = 0;
		int _tmp_bw = 0;
	...
	else
	{
		...
		//_total_cost.push_back((total_cost + 0.1*(host_server_num - dir_host)*serverCost + 2.5 * (dir_host+assign_host_nodes.size()- host_server_num)*serverCost));
		_total_cost.push_back(INF);
		//_total_cost.push_back(((float)(total_cost)) / (consumeNode_nums - assign_host_nodes.size())*(assign_host_nodes.size()*serverCost));
		link_list2.push_back(_total_cost);
		return link_list2;
...
// big
std::vector<std::vector<int>>  Max_Flow_Min_Cost_big(std::vector<netNode_index> &assign_host_nodes)
{
	// 生成动态带宽与租金矩阵
	// N维是服务器源点 N+1维是消费汇点
	std::vector<std::vector<netNode_index>> _flow_cost_bandwidth(
...
int zkw_mcmf(std::vector<netNode_index> &assign_host_nodes)
{
	// 生成动态带宽与租金矩阵
	// N维是服务器源点 N+1维是消费汇点
	std::vector<std::vector<netNode_index>> _flow_cost_bandwidth(
	...
	auto x = costflow(_flow_G_adj, _flow_cost_rentcost, _flow_cost_bandwidth, max_bandwidth_require);
	if (x != INF)
	{
		return (x + assign_host_nodes.size()*serverCost);
	}
	else
		return INF;
	//std::cout << (x + assign_host_nodes.size()*serverCost) << std::endl;
